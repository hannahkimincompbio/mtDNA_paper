...
```{r}
x %>% filter(order == "ACIPENSERIFORMES") %>% ggplot() +geom_density(aes(x = log(alpha), color = gene))
```


```{r}
x %>% ggplot() +geom_density(aes(x = log(alpha), color = gene)) + facet_wrap(vars(order), scales = "free")
```
